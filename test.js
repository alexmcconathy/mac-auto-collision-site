...
['top', 'onsite', 'services', 'process', 'fleet', 'whoworks', 'insurance', 'about', 'gallery', 'reviews', 'faq', 'estimate', 'contact', 'estimateForm', 'formMsg', 'announce', 'aboutText', 'galleryGrid', 'reviewGrid', 'heroHeadline', 'heroSub']
...
const hasAuthenticator = /^\s*base_url:\s*https:\/\/\S+/m.test(cfg);
if (hasAuthenticator) check(/auth_methods:\s*\[[^\]]*oauth[^\]]*\]/.test(cfg), 'admin has an authenticator (base_url), so oauth sign-in is enabled');
else check(/auth_methods:\s*\[\s*token\s*\]/.test(cfg), 'admin shows token sign-in only (no authenticator; the OAuth button would dead-end)');
